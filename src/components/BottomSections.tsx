    ...
    <div className="max-w-3xl mx-auto px-6">
      <motion.div initial={{ opacity: 0, y: 15 }} whileInView={{ opacity: 1, y: 0 }} viewport={{ once: true }} transition={{ duration: 0.6 }}>
        ...
        <div className="flex flex-col sm:flex-row items-center justify-center gap-3">
          <a href={SOLO_PAYMENT_URL} target="_blank" rel="noopener noreferrer" onClick={() => trackCheckout(299)} className="btn-primary inline-block text-center px-10">הזמן עכשיו — ₪299</a>
    ...
    <div className="fixed bottom-0 left-0 right-0 z-[200] md:hidden bg-white border-t border-gray-200 shadow-lg px-4 py-2.5 safe-area-pb" dir="rtl">
      <div className="flex items-center justify-between gap-3">
        ...
        <div className="flex gap-2">
          ...
          <a href={SOLO_PAYMENT_URL} target="_blank" rel="noopener noreferrer" onClick={() => trackCheckout(299)} className="px-5 py-2 bg-primary text-white font-bold text-xs rounded-lg active:scale-95">הזמן</a>
        </div>
      </div>
    </div>
  );
