...
export const FAQ = () => {
  const faqs = [
    { q: "יכול להזיק לרכב?", a: "לא. רק קורא נתונים. אין כתיבה, אין סיכון. צריכת חשמל < 1mA בעמידה." },
    { q: "מה אחרי ההזמנה?", a: "מגיע תוך 3 ימי עסקים. חבר, הורד אפליקציה, סרוק. הכל אוטומטי." },
    { q: "עובד על הרכב שלי?", a: "כמעט בוודאות. כל רכב OBD2 — כמעט כל רכב מ-1996+." },
  ...
  <section className="py-20 md:py-28 bg-dark text-center" dir="rtl">
    <div className="max-w-3xl mx-auto px-6">
      <motion.div initial={{ opacity: 0, y: 15 }} whileInView={{ opacity: 1, y: 0 }} viewport={{ once: true }} transition={{ duration: 0.6 }}>
        <p className="text-primary text-xs font-semibold uppercase tracking-[0.2em] mb-4">One chip. One app. Complete control.</p>
        <h2 className="text-3xl sm:text-5xl font-black text-white tracking-tight mb-4">תפסיק לנחש.<br/>תתחיל לדעת.</h2>
        <p className="text-white/30 text-sm mb-8 max-w-md mx-auto">נהג ממוצע מוציא ₪2,000-₪5,000 בשנה על תיקונים מיותרים. D10 משלם על עצמו בשימוש הראשון.</p>
        ...
        <div className="mt-8 flex flex-wrap justify-center gap-6 text-[10px] font-medium text-white/20">
  ...
  return (
    <div className="fixed bottom-0 left-0 right-0 z-[200] md:hidden bg-white border-t border-gray-200 shadow-lg px-4 py-2.5 safe-area-pb" dir="rtl">
      <div className="flex items-center justify-between gap-3">
        <div><div className="text-surface-dark font-bold text-sm">D10 AI</div><div className="text-gray-400 text-[10px]">₪299 · משלוח חינם</div></div>
...
export const AccessibilityWidget = () => {
  ...
  return (
    <>
      <button onClick={() => setOpen(!open)} className="fixed bottom-5 right-5 z-[999] bg-primary text-white w-11 h-11 rounded-lg shadow-lg flex items-center justify-center hover:bg-primary-dark transition-colors" aria-label="נגישות"><Accessibility size={20} /></button>
      <AnimatePresence>
        {open && (
          <motion.div initial={{ opacity: 0, y: 10 }} animate={{ opacity: 1, y: 0 }} exit={{ opacity: 0, y: 10 }} transition={{ duration: 0.15 }} className="fixed bottom-20 right-5 z-[1000] w-56 bg-white rounded-xl shadow-xl overflow-hidden border border-gray-200" dir="rtl">
